...
Visual themes live in `web/digitalpulse-web/src/theme`. **Executive** (obsidian bioluminescence) is the default dark shell. Editorial, Future AI, and Minimal remain switchable. Components consume CSS tokens from `ThemeProvider`. Theme preference is stored in `localStorage` (`dp.theme` / `dp.theme.user.{userId}`) and is not part of tenant or plan logic. A developer theme switcher is available in `npm run dev`. Pulse scores on the dashboard are identity-record coverage, not invented SEO or review indexes. Header security capsules stay honest (Development JWT, Key Vault hold).
