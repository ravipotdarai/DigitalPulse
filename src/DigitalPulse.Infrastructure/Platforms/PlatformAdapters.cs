...
public abstract class PlatformAdapter : IPlatformAdapter
{
    ...
    public async Task<PlatformMetricsResult> MetricsAsync(PlatformConnection connection, CancellationToken cancellationToken)
    {
...
public sealed class GoogleAdapter(IOfficialPlatformGateway gateway) : PlatformAdapter("GOOGLE", "Google", "Search", PlatformAuthMode.OAuth,
    "Business Profile and Search presence through official Google authorization.",
    new(true, false, true, false, false, true, false), gateway)
{
...
public sealed class FacebookAdapter(IOfficialPlatformGateway gateway) : PlatformAdapter("FACEBOOK", "Facebook", "Social", PlatformAuthMode.OAuth,
    "Page presence through official Meta authorization.",
    new(true, true, true, false, true, true, false), gateway)
{
    protected override (HttpMethod, string, string?, IReadOnlyDictionary<string, string>?)? LiveHealth(PlatformConnection connection) =>
...
public sealed class InstagramAdapter(IOfficialPlatformGateway gateway) : PlatformAdapter("INSTAGRAM", "Instagram", "Social", PlatformAuthMode.OAuth,
...
public sealed class LinkedInAdapter(IOfficialPlatformGateway gateway) : PlatformAdapter("LINKEDIN", "LinkedIn", "Social", PlatformAuthMode.OAuth,
    "Company page through official LinkedIn authorization.",
    new(true, true, false, false, true, true, false), gateway)
{
    protected override (HttpMethod, string, string?, IReadOnlyDictionary<string, string>?)? LiveHealth(PlatformConnection connection) =>
...
public sealed class YouTubeAdapter(IOfficialPlatformGateway gateway) : PlatformAdapter("YOUTUBE", "YouTube", "Social", PlatformAuthMode.OAuth,
...
public sealed class IndiaMartAdapter(IOfficialPlatformGateway gateway) : PlatformAdapter("INDIAMART", "IndiaMART", "Directory", PlatformAuthMode.Assisted,
    "Official CRM lead pull when a seller CRM key is configured. Profile writes stay assisted. Unofficial APIs are out of scope.",
    new(true, false, false, false, false, true, true), gateway)
{
...
public sealed class WhatsAppAdapter(IOfficialPlatformGateway gateway) : PlatformAdapter("WHATSAPP", "WhatsApp", "Messaging", PlatformAuthMode.ApiKey,
    "WhatsApp Business Platform / Cloud API only. Unofficial clients are out of scope.",
    new(false, true, false, false, true, true, false), gateway)
{
    protected override (HttpMethod, string, string?, IReadOnlyDictionary<string, string>?)? LiveHealth(PlatformConnection connection) =>
...
public sealed class SearchConsoleAdapter(IOfficialPlatformGateway gateway) : PlatformAdapter("SEARCH_CONSOLE", "Search Console", "Search", PlatformAuthMode.OAuth,
    "Search visibility through official Search Console authorization.",
    new(true, false, false, false, false, true, false), gateway)
{
    protected override (HttpMethod, string, string?, IReadOnlyDictionary<string, string>?)? LiveHealth(PlatformConnection connection) =>
        (HttpMethod.Get, "https://searchconsole.googleapis.com/webmasters/v3/sites", null, null);
}

public sealed class GoogleAdsAdapter(IOfficialPlatformGateway gateway) : PlatformAdapter("GOOGLE_ADS", "Google Ads", "Ads", PlatformAuthMode.OAuth,
    "Advertising account through official Google Ads authorization.",
    new(true, false, false, false, false, true, false), gateway)
{
    protected override (HttpMethod, string, string?, IReadOnlyDictionary<string, string>?)? LiveHealth(PlatformConnection connection) =>
        (HttpMethod.Get, "https://googleads.googleapis.com/v17/customers:listAccessibleCustomers", null, null);
}
